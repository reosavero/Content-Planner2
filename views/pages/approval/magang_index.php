    ...
    <div class="magang-date-group">
        ...
        <div class="magang-task-grid">
            <?php foreach ($group['tasks'] as $task):
                ...
                $isApproved = in_array($task['status'], ['Approved', 'Selesai', 'Publish']);
            ?>
                <div class="task-card-approval <?= $isApproved ? 'is-approved' : '' ?>" onclick="openTaskSubmitModal(<?= (int)$task['id'] ?>)">
                    <div>
                        <div class="d-flex justify-content-between align-items-start gap-2 mb-2">
                            <div class="task-title" style="margin-bottom:0; flex:1;">
                                <?= htmlspecialchars($task['title'] ?: '(Belum ada judul)') ?>
                            </div>
                            <span class="magang-badge-status <?= $isPending ? 'pending' : 'approved' ?>" style="font-size:10px; padding:3px 8px; flex-shrink:0;">
                                <i class="bi <?= $isPending ? 'bi-clock-history' : 'bi-check-circle-fill' ?>"></i>
                                <?= $isPending ? 'Menunggu Approval' : 'Approved' ?>
                            </span>
                        </div>
                    </div>
